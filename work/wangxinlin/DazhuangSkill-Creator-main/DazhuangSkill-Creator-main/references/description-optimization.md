...
这条路径默认不是“优化已有 skill”的起点，而是两种场景：

- `轻优化` 里的触发层问题
- `完整改造` 完成结构重构之后的后半程

如果你刚接手的是别人的旧 skill，先问自己：它是“根本不触发”，还是“触发后老跑偏”？后者先回 `<skill-base>/references/skill-architecture.md`，先把 skill 拉回主蓝图。
...
换句话说：这份文档解决的是“会不会触发”，不是“该不该先按主蓝图重写”。
...
```bash
<python-cmd> "<skill-base>/scripts/run_loop.py" --eval-set <path-to-trigger-evals> --skill-path <path-to-skill> --model <current-session-model> --max-iterations 5 --verbose
```
...
- 它没有退化成又臭又长的关键词表
- 它仍然清晰、紧凑、可读
- 如果这次属于 `完整改造`，确认新的 description 已经和重构后的主 body 对齐，而不是沿用旧结构时的补丁思路
...
- 单次评估：`<python-cmd> "<skill-base>/scripts/run_eval.py" ...`
- 优化循环：`<python-cmd> "<skill-base>/scripts/run_loop.py" ...`
- description 改写器：`<python-cmd> "<skill-base>/scripts/improve_description.py" ...`
...
- 然后按当前路径直达对应材料：
  ...
  - 如果要跑普通输出评测：先回主 `SKILL.md` 的 Step 1，再看 `<skill-base>/references/eval-planning.md`，确认后再进 `<skill-base>/references/eval-loop.md`
  - 如果要看机器写入 schema：`<skill-base>/references/schemas.md`
  - 如果要直接跑脚本：`<python-cmd> "<skill-base>/scripts/run_eval.py" ...`、`<python-cmd> "<skill-base>/scripts/run_loop.py" ...`
