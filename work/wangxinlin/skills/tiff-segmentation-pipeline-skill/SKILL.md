...
- 优先复用 `<skill-base>/scripts/pipeline_client.py` 与 `<skill-base>/scripts/run_pipeline.py`，不要在会话里重复手写同类调用逻辑。
...
- 收集最小参数：`base_url`、`tiff_path`、`tile_size`、`overlap`、轮询超时。
- 若用户未提供参数，读取 `<skill-base>/config.yaml` 默认值。
- 如只要求排查现有任务，可跳过上传步骤并直接进入任务/结果核验。
